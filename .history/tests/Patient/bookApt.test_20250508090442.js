...
describe('POST /bookApt', () => {
  ...
  it('should return 409 if patient is already booked at that time', async () => {
    ...
    // 👇 More explicit control using .mockImplementationOnce
    aptModel.findOne
      .mockImplementationOnce(() => null) // doctor is available
      .mockImplementationOnce(() => ({ uidPatient: 'patient123' })); // patient has appointment
  
    const res = await request(app)
      .post('/bookApt')
      .set({ uid: 'patient123' })
      .send({
        aptDate: '2025-07-09T10:00:00Z',
        uidDoc: 'doctor456',
        desc: 'Follow-up',
        HName: 'City Hospital',
      });
