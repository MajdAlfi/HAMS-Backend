...
const router = require('../../src/Patient/get/getPatientApt'); 
...
describe('GET /getApt', () => {
  ...
  it('should return appointments for user', async () => {
    ...
    const res = await request(app).get('/getApt').set({ uid: 'test-user-id' });
  
    // Convert Dates to strings for comparison
    const expected = mockAppointments.map((apt) => ({
      uid: apt.uid,
      Apt: apt.Apt.toISOString(),
    }));
